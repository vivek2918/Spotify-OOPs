...
public class Album {
    ...
    public Album(String name, String artist) {
        this.name = name;
        this.artist = artist;
        this.songs = new ArrayList<>(); // allocated memory // if not write then error null pointer exception.
    }
    ...
    public boolean findSong(String title){
        for(Song s:songs){
            if(s.getTitle().equals(title)) // if song title match a title
                return true;
        }
        return false;
    }

    public void addSongToAlbum(String title, double duration){ // parameter pass a song class
        Song s = new Song(title,duration);
        if(findSong(s.getTitle())){ // if true means already added.
